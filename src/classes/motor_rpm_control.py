...
# = 1056
# Calibrated separately per direction -- backward has its own backlash/traction/encoder-edge
# behavior, so it doesn't necessarily match forward's measured steps/rev. If driveRotations(...,
# 'BWD') stops short of the real distance, raise STEPS_PER_WHEEL_REV_BWD (more counted steps
# required per commanded rotation); if it overshoots, lower it.
STEPS_PER_WHEEL_REV_FWD = 125
STEPS_PER_WHEEL_REV_BWD = 175
...
encoder = EncoderRPM(ENCODER_A_PIN, ENCODER_B_PIN, STEPS_PER_WHEEL_REV_FWD)
...
def driveRotations(rotations, pwm, direction):
    ...
    steps_per_rev = STEPS_PER_WHEEL_REV_FWD if direction == 'FWD' else STEPS_PER_WHEEL_REV_BWD
    target_steps = rotations * steps_per_rev
...
def main():
    ...
    try:
        driveRotations(1,80,'FWD')
        driveRotations(1,80,'BWD')
        #print("Here")
        #driveMotor(180,'FWD')
        #time.sleep(5)
